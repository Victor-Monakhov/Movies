...
import {MovieResponse, PageResponse} from "../../models/content";
...
export class ContentService implements OnDestroy {
  ...
  public getFavourites() {
    const favourites: number[] = JSON.parse(localStorage.getItem(this.favouritesKey) as string) || [];
    from(favourites).pipe(
      concatMap(movieId => this.getMovie(movieId)),
      toArray(),
    ).subscribe(movies => {
      this.favourites$.next(movies);
    });
  }
  ...
  public updateFavourites() {
    let favourites: number[] | null = JSON.parse(localStorage.getItem(this.favouritesKey) as string);
    if (favourites) {
      this.favourites$.next(this.favourites$.value.filter(movie => favourites?.find(id => id === movie.id)));
    }
  }
